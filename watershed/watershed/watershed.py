# -*- coding: utf-8 -*-
# Author: 浑水不摸鱼
# Link：https://zhuanlan.zhihu.com/p/153085246
# codes are implemented by Ms student of Michigan University: 浑水不摸鱼
...
def generate_mask(src):
    ...
    # Find total markers
    # contour first output, hierachy second output
    contours, _ = cv.findContours(dist_8u, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE) # modified by zhankun
    # Create the marker image for the watershed algorithm
    markers = np.zeros(dist.shape, dtype=np.int32)
    # Draw the foreground markers
    for i, contour in enumerate(contours): # modified by zhankun
        cv.drawContours(markers, contour, i, (i+1), -1) # modified by zhankun
...
def main():
    input  = './cards.png'
    src = cv.imread(input)
    # src = cv.cvtColor(bgr, cv.COLOR_BGR2GRAY)
